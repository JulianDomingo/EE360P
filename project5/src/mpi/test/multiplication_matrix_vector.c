...
int main(int argc, char **argv) {
  ...
  int *chunk_sizes = malloc(sizeof(int) * number_of_processes);
  int *starting_index_of_chunks = malloc(sizeof(int) * number_of_processes);

  int *received_chunk_sizes = malloc(sizeof(int) * number_of_processes);
  int *received_starting_index_of_chunks = malloc(sizeof(int) * number_of_processes);
  ...
  if (is_root_process(process_rank)) {
    ...
    temporary_vector = malloc(sizeof(int) * 1000);
  ...
  if (process_rank < matrix_row_size) {
    ...
    if (is_root_process(process_rank)) {
      ...
      fclose(matrix_file);
      resultant_vector = malloc(sizeof(int) * matrix_column_size);
    }
    ...
    if (is_root_process(process_rank)) {
      // MPI_Type_vector: creates an MPI vector based on 
      ...
      MPI_Gatherv(send_buffer, 1, sending_type, resultant_vector, received_chunk_sizes, received_starting_index_of_chunks, MPI_INT, 0,
                  MPI_COMM_WORLD);
    }
